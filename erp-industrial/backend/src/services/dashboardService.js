...
export const dashboardService = {
  async getResumo() {
    const [producao, vendas, baixos, alertas] = await Promise.all([
      pool.query(
        `SELECT COUNT(*)::int AS total FROM ordens_producao WHERE status IN ('pendente','em_producao','aguardando_pecas')`,
      ),
      pool.query(`SELECT * FROM vendas ORDER BY id DESC LIMIT 5`),
      pool.query(
